import { useEffect, useState } from 'react';
import { Link } from 'react-router-dom';
import api from '../api/client.js';
import { useAuth } from '../context/AuthContext.jsx';
import { useCart } from '../context/CartContext.jsx';

const WISHLIST_KEY = 'chhatak_wishlist';

const TONE_MAP = {
  'Indian Classic': 'tone-warm',
  'Peri Peri Blaze': 'tone-fire',
  'Mint & Lime': 'tone-cool',
};

const PRODUCT_IMAGES = {
  'indian-classic': '/images/bowl.JPG',
  'peri-peri-blaze': '/images/packaging-real.JPG',
  'combo-3x-classic': '/images/packaging-front-back.png',
};

function readWishlist() {
  try { return new Set(JSON.parse(localStorage.getItem(WISHLIST_KEY) || '[]')); }
  catch { return new Set(); }
}

function writeWishlist(set) {
  localStorage.setItem(WISHLIST_KEY, JSON.stringify([...set]));
}

export default function WishlistPage() {
  const [products, setProducts] = useState([]);
  const [wishlist, setWishlist] = useState(() => readWishlist());
  const [loading, setLoading] = useState(true);
  const [addingId, setAddingId] = useState(null);
  const { isAuthenticated } = useAuth();
  const { addToCart } = useCart();

  useEffect(() => {
    api.get('/products/')
      .then(({ data }) => setProducts(data))
      .catch(() => setProducts([]))
      .finally(() => setLoading(false));
  }, []);

  const remove = (id) => {
    setWishlist((prev) => {
      const next = new Set(prev);
      next.delete(id);
      writeWishlist(next);
      return next;
    });
  };

  const handleAddToCart = async (productId) => {
    if (!isAuthenticated) {
      window.location.href = '/login';
      return;
    }
    setAddingId(productId);
    try {
      await addToCart(productId);
    } catch (err) {
      alert(err.response?.data?.detail || 'Failed to add to cart');
    } finally {
      setAddingId(null);
    }
  };

  const items = products.filter((p) => wishlist.has(p.id));

  return (
    <div className="section wishlist-page" style={{ paddingTop: 140 }}>
      <div className="container">
        <p className="kicker">— Wishlist</p>
        <h2 className="display sm">Saved for <em>later</em>.</h2>

        {loading ? (
          <p style={{ color: 'var(--muted)', marginTop: 40 }}>Loading…</p>
        ) : items.length === 0 ? (
          <div style={{ marginTop: 40, padding: '60px 24px', textAlign: 'center', background: 'var(--bg-soft)', border: '1px solid var(--line)', borderRadius: 8 }}>
            <p style={{ color: 'var(--muted)', marginBottom: 20 }}>Your wishlist is empty. Tap the heart on any product to save it here.</p>
            <Link to="/#products" className="btn-solid accent">Browse products →</Link>
          </div>
        ) : (
          <div className="flavor-grid" style={{ marginTop: 40 }}>
            {items.map((product, i) => (
              <article className="flavor-card" key={product.id}>
                <div className={`flavor-thumb ${TONE_MAP[product.flavor] || 'tone-warm'}`}>
                  <Link to={`/products/${product.slug}`} className="flavor-thumb-link">
                    {PRODUCT_IMAGES[product.slug] && (
                      <img src={PRODUCT_IMAGES[product.slug]} alt={product.name} className="flavor-img" />
                    )}
                    <span className="flavor-label">{String(i + 1).padStart(2, '0')} / {product.flavor || product.name}</span>
                  </Link>
                  <button
                    type="button"
                    className="wishlist-btn is-active"
                    onClick={() => remove(product.id)}
                    aria-label="Remove from wishlist"
                  >
                    <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                      <path d="M20.4 5.6a5.5 5.5 0 0 0-7.8 0L12 6.2l-.6-.6a5.5 5.5 0 1 0-7.8 7.8l.6.6L12 21.8l7.8-7.8.6-.6a5.5 5.5 0 0 0 0-7.8Z" />
                    </svg>
                  </button>
                </div>
                <div className="flavor-body">
                  <Link to={`/products/${product.slug}`}>
                    <h3>{product.name}</h3>
                  </Link>
                  <p>{product.description}</p>
                  <div className="flavor-meta">
                    <span>
                      ₹{product.price}
                      {product.compare_at_price && (
                        <span className="price-was" style={{ marginLeft: 8, fontSize: 14 }}>₹{product.compare_at_price}</span>
                      )}
                    </span>
                    {product.stock > 0 ? (
                      <button
                        className="btn-solid accent"
                        onClick={() => handleAddToCart(product.id)}
                        disabled={addingId === product.id}
                      >
                        {addingId === product.id ? 'Adding…' : 'Add to bag →'}
                      </button>
                    ) : (
                      <span className="out-of-stock-label">Out of stock</span>
                    )}
                  </div>
                </div>
              </article>
            ))}
          </div>
        )}
      </div>
    </div>
  );
}
