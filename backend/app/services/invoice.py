...
def _latin1(s: str) -> str:
    """Reportlab's Helvetica is Latin-1 only; swap common Unicode punctuation
    that would otherwise render as a black box in the PDF."""
    if not s:
        return s
    return (
        s.replace("—", " - ")   # em dash
...
def generate_invoice_pdf(order: Order) -> bytes:
    ...
    brand = ParagraphStyle("Brand", parent=styles["Title"], fontSize=28, textColor=colors.HexColor("#F0994A"), fontName="Helvetica-Bold")
    ...
    if order.shipping_address:
        ...
        for line in order.shipping_address.split("\n"):
            elements.append(Paragraph(_latin1(line), styles["Normal"]))
        elements.append(Spacer(1, 8 * mm))
    ...
    for i, item in enumerate(order.items, 1):
        name = _latin1(item.product.name if item.product else f"Product #{item.product_id}")
        table_data.append([str(i), name, str(item.quantity), f"Rs.{item.unit_price:.2f}", f"Rs.{item.total_price:.2f}"])

    table_data.append(["", "", "", "Subtotal", f"Rs.{sum(i.total_price for i in order.items):.2f}"])
    shipping = 49 if order.total_amount < 499 else 0
    table_data.append(["", "", "", "Shipping", f"Rs.{shipping:.2f}" if shipping else "Free"])
    table_data.append(["", "", "", "Total", f"Rs.{order.total_amount:.2f}"])
    ...
    method = "Cash on Delivery" if getattr(order, "payment_method", None) == "cod" else "Online (Razorpay)"
    elements.append(Paragraph(f"Payment: {method}", small))
    elements.append(Paragraph(f"Status: {order.status.replace('_', ' ').title()}", small))
    elements.append(Spacer(1, 20 * mm))
    elements.append(Paragraph(_latin1("Thank you for shopping with Chhatak — The Coastal Crunch!"), center))
